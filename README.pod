...
- make sure that the file list only contains unique values so I don't get race conditions

Niceties:

- switch to specify output file (-o)

  - automatically dump to s/docx$/txt/, and have -o specify different one? - no
